      ...
      else
      {
        ...
        else
        {
          if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file1))
          {
            ...
            $result="<div class='alert alert-success'>L'utilisateur a bien été ajouté</div>";
          }
          else
          {
            $result="<div class='alert alert-danger'>Désolé, nous avons rencontre un problème lors du téléchargement de votre photo veuillez recommencer</div>";
        ...
        <div class="box-body">
          <form method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <input type="date" class="form-control" name="datenais" placeholder="mm/dd/yyyy" required>
            </div>
      ...
      <div class="box box-solid box-default">
        ...
        <div class="box-body no-padding">
          <ul class="users-list clearfix">
            <?php
              $messagesParPage=8;
              $total=$app->getTable('Utilisateur')->compte()->tot;
              ...
              foreach ($app->getTable('Utilisateur')->afficherPage($premiereEntree, $messagesParPage) as $user) {
                echo'
                <li>
                  <img src="'.$user->lien.'" alt="User Image">
                  <a class="users-list-name" href="'.$user->getUrl().'">'.$user->nom.' '.$user->prenom.'</a>
                    <span class="users-list-date">';
                        echo App::ctrlDate($user);
               echo'</span>
               <form action="admin.php?p=userdelete" method="post" style="color:red ">
                 <input type="hidden" name="id" value='.$user->id.'>
                 <button type="submit" href="admin.php?p=userdelete&id="'.$user->id.' ><i class="fa fa-times"></i></button>
               </form>
                </li>
                ';
              }
          ...
          <ul class="pagination">
            ...
            for($i=1; $i<=$nombreDePages; $i++) //On fait notre boucle
            {
                if($i==1)
                {
                    ...
                    {
                        echo '<li class="prev"><a href="admin.php?p=users&page='.($pageActuelle-1).'"><span class="fa fa-angle-left"></span>Prev</a></li>';
                    }

                }
                if($i==$pageActuelle) echo '<li class="active"><a href="#">'.$i.'</a></li>';
                else echo '<li><a href="admin.php?p=users&page='.$i.'">'.$i.'</a></li>';
                if($i==$nombreDePages)
                {
                    ...
                    {
                        echo '<li class="next"><a href="admin.php?p=users&page='.($pageActuelle+1).'">Next<span class="fa fa-angle-right"></span></a></li>';
                    }
